...
static void esc_address(uint16_t address, uint8_t command)
{
   ...
   spi_bidirectionally_transfer(ax58100, (uint8_t *)&ESCvar.ALevent,
                                data, sizeof(data));
...
void ESC_read(uint16_t address, void *buf, uint16_t len)
{
    ...
    */
   spi_bidirectionally_transfer(ax58100, buf, read_termination + (MAX_READ_SIZE - len), len);

   /* Un-select device. */
   spi_unselect(ax58100);
}
...
void ESC_init(const esc_cfg_t *config)
{
   ...
   // {
   //    ESC_read(ax58100, (void *)&device_symbol, sizeof(uint8_t));
